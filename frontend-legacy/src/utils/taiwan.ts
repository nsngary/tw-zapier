 ...
 * 解析新台幣金額字串為分
 */
export const parseTWDAmount = (amountString: string): TWDAmount => {
  // 移除貨幣符號、逗號和空格
 ...
 * 格式化台灣日期時間
 */
export const formatTaiwanDateTime = (
  date: Date, 
 ...
 * 解析台灣日期字串
 */
export const parseTaiwanDate = (dateString: string, isRepublicYear = false): Date => {
  // 移除星期資訊
 ...
 * 格式化台灣地址
 */
export const formatTaiwanAddress = (address: TaiwanAddress): string => {
 ...
 * 解析台灣地址
 */
export const parseTaiwanAddress = (addressString: string): Partial<TaiwanAddress> => {
  // 簡單的地址解析邏輯
  ...
  const taiwanCities = Object.values(TaiwanCity)
  for (const taiwanCity of taiwanCities) {
 ...
 * 取得支付方式顯示名稱
 */
export const getPaymentMethodDisplayName = (method: TaiwanPaymentMethod): string => {
 ...
 * 取得銀行名稱
 */
export const getBankName = (bankCode: TaiwanBankCode): string => {
 ...
 * 取得超商名稱
 */
export const getConvenienceStoreName = (store: TaiwanConvenienceStore): string => {
 ...
 * 產生台灣時間戳
 */
export const generateTaiwanTimestamp = (): string => {
  return formatTaiwanDateTime(new Date(), TaiwanDateFormat.YYYY_MM_DD_HH_MM_SS)
}

/**
 * 產生隨機訂單編號
 */
export const generateOrderId = (prefix = 'TW'): string => {
 ...
 * 檢查是否為台灣工作日
 */
export const isTaiwanWorkingDay = (date: Date): boolean => {
  const taiwanDate = new Date(date.toLocaleString('en-US', { timeZone: TAIWAN_TIMEZONE }))
 ...
 * 取得下一個台灣工作日
 */
export const getNextTaiwanWorkingDay = (date: Date): Date => {
  const nextDay = new Date(date)
  nextDay.setDate(nextDay.getDate() + 1)
  
  while (!isTaiwanWorkingDay(nextDay)) {
    nextDay.setDate(nextDay.getDate() + 1)
  }
 ...
 * 計算台灣工作日差異
 */
export const calculateTaiwanWorkingDays = (startDate: Date, endDate: Date): number => {
  let workingDays = 0
  const currentDate = new Date(startDate)
  
  while (currentDate <= endDate) {
    if (isTaiwanWorkingDay(currentDate)) {
      workingDays++
    }
