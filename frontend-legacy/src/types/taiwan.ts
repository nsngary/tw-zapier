...
export interface TaiwanNodeConfig {
  ...
  locale: string
  currency: string
  dateFormat: TaiwanDateFormat
  enableValidation: boolean
  validationRules: TaiwanValidationRules
}

/**
 * Line Pay 配置
 */
export interface LinePayConfig extends TaiwanNodeConfig {
  channelId: string
 ...
 * ECPay 配置
 */
export interface ECPayConfig extends TaiwanNodeConfig {
  merchantId: string
  hashKey: string
  hashIV: string
  testMode: boolean
 ...
 * 政府開放資料配置
 */
export interface GovOpenDataConfig extends TaiwanNodeConfig {
  apiKey?: string
  baseUrl: string
  timeout: number
  retryCount: number
 ...
 * 政府開放資料回應
 */
export interface GovOpenDataResponse {
  success: boolean
  result: {
    resource_id: string
    limit: number
